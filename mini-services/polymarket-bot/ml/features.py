...
def extract_features(market: dict, book: OrderBook) -> np.ndarray | None:
    ...
    try:
        ...
        fund_sentiment = 0.0
    # R14: Derive competitiveness from spread (Gamma never returns "competitive",
    # so the previous `market.get("competitive") or 0.9` was a constant 0.9 in
    # production vs uniform(-1,1) in training — a train/serve skew. Now derived
    # from the live spread so train (synthetic spread) and serve (real spread)
    # produce comparable distributions in [-1, 1].
    spread_for_comp = max(spread, 0.001)
    competitiveness = float(np.clip(1.0 - (spread_for_comp / 0.05), -1.0, 1.0))
