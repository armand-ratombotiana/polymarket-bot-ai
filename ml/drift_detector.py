...
class ModelDriftDetector:
    ...
    Drift is flagged by three independent signals (any one suffices):
      1. PSI ≥ 0.20  — prediction distribution shifted vs. market baseline
    ...
    def compute_psi(self) -> float:
        ...
        expected = self.baseline_distribution + 1e-10
        ...
        if self.last_psi < 0.10 and self.last_ks_stat < 0.15:
            new_status = "HEALTHY"
        elif self.last_psi < 0.20 and self.last_ks_stat < 0.25:
    ...
    def reset(self) -> None:
        ...
        log.info("[drift_detector] Rolling window reset after model re-train")
